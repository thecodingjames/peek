// Source
// https://github.com/hschneider/neutralino-curl/blob/master/README.md
...
export class Curl {
    constructor(opt= {}) {
        ...
        // NL_RESMODE
        const bin = `nl://src/vendor/curl/${NL_OS}-${NL_ARCH}`.toLowerCase()
        this.appResourcesEXT = NL_PATH + bin
        if(NL_OS === 'Windows' ) {
            this.appResourcesEXT = NL_CWD + bin
        }

        // wip
        this.appResourcesEXT = bin
    ...
    async run(args) {
        ...
        alert(`${this.appResourcesEXT}/bin/curl ${args}`)
        // let cmd = await Neutralino.os.spawnProcess(`${this.appResourcesEXT}/bin/curl ${args}`);
        let cmd = await Neutralino.os.spawnProcess(`ls ./`);
